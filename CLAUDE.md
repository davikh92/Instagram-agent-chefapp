...
| `docs/MAPA-DO-PROJETO.md` | **O contrato de diretórios** — plataforma nova nasce em `scripts/<p>/`, `data/<p>/`, `templates/<p>/`, `assets/<p>/` e não escreve fora deles. Instagram é o legado na raiz (migração agendada ~04–05/09, pós-estreia). **Consultar antes de criar qualquer arquivo novo.** |
