...
- **Reels são gerados pelo Gemini Omni Flash** (`generate-omni.js`) — 10s, 9:16, ~$1/vídeo, som ambiente (`No speech...`). Veo 3.1 Lite/Fast ficam como alavanca de custo. **Texto no vídeo: decisão do Davi (27/08) — overlay ffmpeg está DESCARTADO.** Ele já testou esse tipo de edição e o resultado estraga o vídeo. O caminho a testar é pedir a assinatura/números **na própria geração** (1 vídeo de teste). Até dar certo, os reels saem sem texto — e isso não bloqueia publicação.
